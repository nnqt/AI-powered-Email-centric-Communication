...
\begin{itemize}
    \item \textbf{FR-04 – Cập nhật giao diện tức thời (real-time UI update):}
...
Module AI vận hành như một \textit{AI microservice} độc lập, giao tiếp với backend qua HTTP/REST hoặc WebSocket.

\begin{itemize}
    \item \textbf{FR-05 – Tự động khởi tạo và làm giàu hồ sơ Contact:}
    Khi xuất hiện một địa chỉ email mới trong luồng dữ liệu, backend chỉ ghi nhận tối thiểu thông tin kỹ thuật (email, thời gian, header), còn AI service sẽ chạy nền để suy luận thêm metadata: tên hiển thị, ngôn ngữ thường sử dụng, domain liên quan tới tổ chức nào,… Từ đó, hệ thống tạo hoặc cập nhật một hồ sơ Contact ở trạng thái sơ bộ (draft) mà không cần thao tác thủ công từ người dùng.

    \item \textbf{FR-06 – Gợi ý hợp nhất định danh (AI-assisted Contact Unification):}
    Dựa trên lịch sử giao tiếp, nội dung email và các đặc trưng như domain, chữ ký, cách xưng hô, AI service đưa ra các gợi ý nhóm nhiều địa chỉ email khác nhau vào cùng một Contact duy nhất. Người dùng có thể xem danh sách gợi ý này trong giao diện và chấp nhận hoặc từ chối. Sau khi được chấp nhận, thao tác hợp nhất được thực hiện nền, đảm bảo toàn bộ timeline và dữ liệu liên quan được quy về một hồ sơ "360 độ".

    \item \textbf{FR-07 – Tóm tắt luồng trao đổi (Thread Summarization):}
    Đối với các chuỗi email dài (thread), hệ thống định kỳ hoặc theo yêu cầu gửi nội dung thread tới AI Service để sinh ra một bản tóm tắt ngắn gọn (summary) kèm theo danh sách "Key Issues". Bản tóm tắt này được lưu lại trong DB, gắn với thread tương ứng và hiển thị trực tiếp trên giao diện Timeline để người dùng nắm nhanh "bức tranh tổng thể".

    \item \textbf{FR-08 – Gợi ý phản hồi thông minh (Smart Reply Suggestion):}
    Khi người dùng mở một email gần nhất trong thread, AI Service dựa trên nội dung email hiện tại cộng với ngữ cảnh lịch sử để đề xuất 2–3 phương án trả lời (reply candidates). Các gợi ý này nên ở dạng có thể chỉnh sửa lại trước khi gửi, đảm bảo vai trò AI là hỗ trợ (augmented intelligence) chứ không thay thế hoàn toàn con người.
\end{itemize}

\subsubsection*{(5) Mở rộng đa kênh và kiến trúc adapter}

Mặc dù PoC chỉ tích hợp Email, kiến trúc cần sẵn sàng cho việc bổ sung các kênh IM.

\begin{itemize}
    \item \textbf{FR-10 – Định nghĩa interface chuẩn cho Message/Conversation:}
...
\subsection{Quyết định công nghệ và Chiến lược kiến trúc}
\label{subsec:GiaiPhapCongNghe}

Dựa trên yêu cầu ở trên, luận án đưa ra một số quyết định công nghệ và chiến lược kiến trúc trọng yếu.
...
\begin{itemize}
    \item Tránh làm nghẽn luồng xử lý của backend khi AI phải gọi các API có độ trễ cao (OpenAI).
