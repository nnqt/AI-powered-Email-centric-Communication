...
// Get base URL from environment – fall back to localhost so build-time
// module evaluation doesn't throw; real value must be set at runtime.
...
axiosClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    logger.debug(
      `[${requestId}] Request: ${config.method?.toUpperCase()} ${config.url}`,
      {
